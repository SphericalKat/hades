...
func (pr *PostgresRepository) FindByRegNo(regNo string) (*entity.Participant, error) {
	p := &entity.Participant{}
	err := pr.db.Where("registration_number = ?", regNo).Find(p).Error
	return p, err
}

func (pr *PostgresRepository) Save(p *entity.Participant) error {
	return pr.db.Save(p).Error
}

func (pr *PostgresRepository) Delete(regNo string) error {
	p := &entity.Participant{RegistrationNumber: regNo}
